...
func (r *rekeningRepository) IncrementSaldo(ctx context.Context, id uuid.UUID, userId uuid.UUID, saldo int) (rekeningModel.Rekening, int64, error) {
	var rekening rekeningModel.Rekening

	tx := r.db.WithContext(ctx).Model(&rekeningModel.Rekening{}).Where("id = ? AND user_id = ?", id, userId).Update("saldo", gorm.Expr("saldo + ?", saldo))
