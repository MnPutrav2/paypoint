...
func (s *orderService) GetGrafikOmzet(ctx context.Context, mode utilConst.GrafikMode) ([]dashboardModel.Grafik, error) {
	...
	go func() {
		v, err := s.repo.GetGrafikOmzet(ctx, from, to, mode)
